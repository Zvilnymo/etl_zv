...
_UPSERT_SQL = """
    INSERT INTO crm.fact_leads (
        ...
        call_status, lead_substatus, qualification_level,
        rejection_reason, closure_comment,
        is_repeated, total_debt, banks_count, banks_debt,
        mfo_count, mfo_debt, official_income, creditors_count,
        callback_at, is_closed, planned_close_date, phone,
        etl_loaded_at
    ) VALUES (
        %(id)s, %(status_id)s, %(date_create)s, %(date_modify)s, %(manager_id)s,
        %(source_id)s, %(source_description)s,
        %(taken_in_work_at)s, %(time_taken_in_work_sec)s,
        %(utm_source)s, %(utm_medium)s, %(utm_campaign)s, %(utm_content)s, %(utm_term)s,
        %(call_status)s, %(lead_substatus)s, %(qualification_level)s,
        %(rejection_reason)s, %(closure_comment)s,
        %(is_repeated)s, %(total_debt)s, %(banks_count)s, %(banks_debt)s,
        %(mfo_count)s, %(mfo_debt)s, %(official_income)s, %(creditors_count)s,
        %(callback_at)s, %(is_closed)s, %(planned_close_date)s, %(phone)s,
        NOW()
    )
